...
class Player:
    def __init__(self, name):
        ...
        self.suits = '♣♦♥♠'
        self.dict_cards = {card: val for val, card in enumerate(self.generate_card_strings())}
...
#-----------------------------------card to index-----------------------------------#
    ...
    # Generate a list of card strings in the desired order
        card_strings = []
...
class Pokergame:
  def __init__(self):
    ...
    self.list_of_players = [self.Player, self.Bot, self.Agent]
    ...
    match self.round_counter:
      case 1:
        for player in self.list_of_players_not_out:
          ...
          print(player.cards)
          card1 = player.to_index(player.cards[0])
          card2 = player.to_index(player.cards[1])
          print(f"Card1 = {card1}")
          print(f"Card2 = {card2}")
        if card1 < 10:
            card1 = "0" + str(card1) + "P"
        elif str(card1).isdigit() and int(card1) >= 10:
            card1 = str(card1) + "P"
        if card2 < 10:
            card2 = "0" + str(card2) + "P"
        elif str(card2).isdigit() and int(card2) >= 10:
            card2 = str(card2) + "P"
        # print(f"after {card1}")
        # print(f"after {card2}")
        subprocess.run(["python", "SendCardIndex.py", "--card", card1])
        subprocess.run(["python", "SendCardIndex.py", "--card", card2])
        
        currentPot = self.pot
        subprocess.run(["python", "getPot.py", "--pot", str(currentPot)])
        # return player.cards
        self.act_one()
    ...
    # In the second round, the dealer draws three community cards and adds them to the 'cards' list.
      case 2:
        ...
        for player in self.list_of_players_not_out:
          self.cards = self.deck.draw(3)
          player.print_round_info(self.pot, self.cards)
          card1 = player.to_index(self.cards[0])
          card2 = player.to_index(self.cards[1])
          card3 = player.to_index(self.cards[2])
        ...
        elif str(card3).isdigit() and int(card3) >= 10:
            card3 = str(card3) + "B"
        ...
        subprocess.run(["python", "SendCardIndex.py", "--card", card1])
        subprocess.run(["python", "SendCardIndex.py", "--card", card2])
        subprocess.run(["python", "SendCardIndex.py", "--card", card3])
...
#-----------------------------------clear board-----------------------------------#
  def reset_game():
      subprocess.run(["python", "restartGame.py"], capture_output=True, text=True)
#-----------------------------------end game-----------------------------------#
  ...
  def end_round(self):
    ...
    if len(self.list_of_players_not_out) == 1:
        ...
        #subprocess.run(["python", "winner.py", "--win", winning_player])
        quit()
    ...
    subprocess.run(["python", "winner.py", "--win", self.winners[0].name])
    
    result = self.reset_game()
    
    new_round = result.stdout.strip()
    ...
    if len(self.fold_list) == (len(self.list_of_players_not_out)):
        for remaining_player in self.list_of_players_not_out:
            if remaining_player not in self.fold_list:
                ...
                subprocess.run(["python", "winner.py", "--win", str(remaining_player.name)])
...
##-----------------------------------bet or raise----------------------------------##
  ...
  def handle_bet_or_raise(self, player):
    ...
    self.turn_counter += 1
    # player.past_turn(self.turn_counter)
    #bet = subprocess.check_output(["python", "GetPlayerRaise.py"]).decode().strip()
    bet = int(input("Enter your raise: "))
    print(f"Player Raised {bet}")
...
##-----------------------------------answer----------------------------------##
  def answer(self, player):
    ...
    result = subprocess.run(["python", "playerActions.py"], capture_output=True)
    response = result.stdout.decode("utf-8").strip()
    if response.endswith("\n"):
        response = response[:-1]  # Remove the last character if it is a newline character
    self.response = response
    # print(f" response = {self.response}")
    # print(f" possible_responses = {self.possible_responses}")
    # print(f" type = {type(self.response)}")
    # if str(self.response) not in self.possible_responses:
    #     print("Invalid response \n-----------------------------------")
    #     continue
    
    if self.response == "all_in":
        print(f"Received all-in!")
        self.handle_all_in(player)
        return True
    elif self.response == "fold":
        print(f"Received fold!")
