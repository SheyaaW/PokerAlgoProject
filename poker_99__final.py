# -*- coding: utf-8 -*-
"""Poker_99%_final.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1isgTlok16pdnY_8Xrvs1OXMoOUN1Noi3
...
class Pokergame:
  def __init__(self):
    ...
    self.ready_list = []
    self.check = 0
    self.Agent1 = Player('Agent1')
    self.Agent2 = Player('Agent2')
    self.Agent3 = Player('Agent3')
    self.list_of_players = [self.Agent1, self.Agent2, self.Agent3]
  ...
  def dealer(self):
    ...
    match self.round_counter:
      case 1:
        for player in self.list_of_players_not_out:
          ...
          player.hole = 1
          player.get_agent_cards(player.cards)
          player.rank = 0
    ...
    # In the second round, the dealer draws three community cards and adds them to the 'cards' list.
      case 2:
        ...
        for player in self.list_of_players_not_out:
          player.get_agent_cards(self.cards)
          self.get_in_ranks(ranks, player)
          player.print_round_info(self.pot, self.cards)
    ...
    # For 3-4 round , the dealer draws one additional community card and appends it to the 'cards' list.
      case 3:
        ...
        ranks = EV.flop_evaluate(self.cards,self.list_of_players_not_out)
        for player in self.list_of_players_not_out:
          player.get_agent_cards([self.cards[self.round_counter]])
          self.get_in_ranks(ranks, player)
          player.print_round_info(self.pot, self.cards)
      case 4:
        ranks = []
...
##-----------------------------------answer----------------------------------##
  def answer(self, player):
    ...
    self.response = action_choices[act]
